...
$total_semua = (int)$pdo->query("SELECT COUNT(*) FROM users WHERE role IN ('mahasiswa','admin') AND status='aktif'")->fetchColumn();
$total_mhs   = (int)$pdo->query("SELECT COUNT(*) FROM users WHERE role='mahasiswa' AND status='aktif'")->fetchColumn();
$total_admin = (int)$pdo->query("SELECT COUNT(*) FROM users WHERE role='admin' AND status='aktif'")->fetchColumn();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($aksi === 'broadcast') {
        ...
        if (!$judul || !$pesan) {
            ...
            // Ambil daftar user sesuai target
            if ($target === 'mahasiswa') {
                $users = $pdo->query("SELECT id FROM users WHERE role='mahasiswa' AND status='aktif'")->fetchAll(PDO::FETCH_COLUMN);
            } elseif ($target === 'admin') {
                $users = $pdo->query("SELECT id FROM users WHERE role='admin' AND status='aktif'")->fetchAll(PDO::FETCH_COLUMN);
            } else {
                $users = $pdo->query("SELECT id FROM users WHERE role IN ('mahasiswa','admin') AND status='aktif'")->fetchAll(PDO::FETCH_COLUMN);
            }

            try {
                $pdo->beginTransaction();

                // INSERT notifikasi satu per satu dengan ID manual
                $count = 0;
                foreach ($users as $uid) {
                    $newId = (int)$pdo->query("SELECT COALESCE(MAX(id),0)+1 FROM notifikasi")->fetchColumn();
                    $pdo->prepare("INSERT INTO notifikasi (id, user_id, judul, pesan, tipe, created_at) VALUES (?, ?, ?, ?, ?, NOW())")
                        ->execute([$newId, $uid, $judul, $pesan, $tipe]);
                    $count++;
                }

                // INSERT broadcast_log dengan ID manual
                $newLogId = (int)$pdo->query("SELECT COALESCE(MAX(id),0)+1 FROM broadcast_log")->fetchColumn();
                $pdo->prepare("INSERT INTO broadcast_log (id, user_id, judul, pesan, target, tipe, jumlah_penerima, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())")
                    ->execute([$newLogId, $_SESSION['user_id'], $judul, $pesan, $target, $tipe, $count]);

                // INSERT activity_log dengan ID manual
                $newActId = (int)$pdo->query("SELECT COALESCE(MAX(id),0)+1 FROM activity_log")->fetchColumn();
                $pdo->prepare("INSERT INTO activity_log (id, user_id, role, aksi, detail, ip_address) VALUES (?,?,?,?,?,?)")
                    ->execute([$newActId, $_SESSION['user_id'], 'super_admin', 'Broadcast', "Kirim ke $count pengguna: $judul", $_SERVER['REMOTE_ADDR']]);

                $pdo->commit();
                $success = "Broadcast berhasil dikirim ke $count pengguna!";
            } catch (Exception $e) {
                $pdo->rollBack();
                $error = 'Gagal kirim broadcast: ' . $e->getMessage();
            }
        }
    }
    ...
    if ($aksi === 'hapus_log') {
        ...
        if ($log_id > 0) {
            $pdo->prepare("DELETE FROM broadcast_log WHERE id=?")->execute([$log_id]);
            $success = 'Log broadcast berhasil dihapus.';
        }
    }
}
...
<div class="main-grid fu3">
  ...
  <div class="log-card">
    ...
    <div class="log-list">
      ...
      <div class="log-item">
        <div>
          <span class="log-tipe-badge lt-<?= e($log['tipe']) ?>">
            <i class="fas <?= $tipe_ico ?>"></i> <?= e($log['tipe']) ?>
          </span>
        </div>
        ...
        <div class="log-meta">
          <div class="log-meta-left">
            ...
            <span><i class="fas fa-users" style="font-size:9px;"></i> <?= (int)$log['jumlah_penerima'] ?> penerima</span>
          </div>
          <form method="POST" style="display:inline;">
            <input type="hidden" name="aksi" value="hapus_log">
            <input type="hidden" name="log_id" value="<?= (int)$log['id'] ?>">
            <button type="submit" class="log-del" onclick="return confirm('Hapus log ini?')">
