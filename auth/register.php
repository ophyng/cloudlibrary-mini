...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($password!==$konfirm) { $error='Konfirmasi password tidak cocok.'; }
        ...
        if ($cek->fetch()) { $error='Email sudah terdaftar.'; }
            ...
            $newId = (int)($pdo->query("SELECT COALESCE(MAX(id),0)+1 FROM users")->fetchColumn());
            $pdo->prepare("INSERT INTO users (id,nama,email,password,role,status,poin,created_at) VALUES (?,?,?,?,?,?,?,NOW())")->execute([$newId,$nama,$email,md5($password),'mahasiswa','aktif',0]);
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .reg-wrap{position:relative;z-index:2;display:flex;width:900px;max-width:96vw;border-radius:28px;overflow:hidden;box-shadow:0 32px 80px rgba(0,0,0,0.45),0 8px 24px rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.14);animation:popIn .5s cubic-bezier(.34,1.56,.64,1) forwards;}
    @keyframes popIn{from{opacity:0;transform:scale(.93)}to{opacity:1;transform:scale(1)}}
    .reg-left{flex:1.1;padding:48px 44px;display:flex;flex-direction:column;justify-content:center;background:rgba(255,255,255,0.78);backdrop-filter:blur(28px);-webkit-backdrop-filter:blur(28px);border-right:1px solid rgba(255,255,255,0.40);}
    ...
    .input-wrap input{width:100%;background:rgba(255,255,255,0.90);border:1.5px solid rgba(30,74,130,0.18);border-radius:12px;padding:11px 14px 11px 40px;color:var(--d1);font-size:14px;font-weight:600;font-family:'Nunito',sans-serif;transition:border-color .2s,box-shadow .2s;outline:none;}
    ...
    .reg-right{flex:1;background:url('gambar_library.jpg') center center / cover no-repeat;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between;padding:44px 36px;min-height:560px;}
    .reg-right::before{content:'';position:absolute;inset:0;z-index:0;background:linear-gradient(160deg,rgba(14,30,60,0.88) 0%,rgba(20,50,90,0.78) 50%,rgba(30,74,130,0.70) 100%);}
    .reg-right::after{content:'';position:absolute;inset:12px;z-index:1;border:1.5px dashed rgba(255,255,255,0.15);border-radius:18px;pointer-events:none;}
    ...
    .rr-brand-ico{width:36px;height:36px;border-radius:10px;background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.28);display:flex;align-items:center;justify-content:center;color:#fff;font-size:15px;}
    ...
    .step-ico{width:28px;height:28px;border-radius:8px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:13px;color:#fff;}
    ...
    @media(max-width:720px){.reg-right{display:none;}.reg-left{padding:36px 24px;}}
  </style>
</head>
<body>
<div class="reg-wrap">
  <div class="reg-left">
    ...
    <form method="POST" autocomplete="off">
      ...
      <button type="submit" class="btn-reg"><i class="fas fa-user-plus"></i> Daftar Sekarang</button>
    </form>
      ...
      <div class="rr-heading">
        ...
        <div class="steps">
          <div class="step">
            ...
            <div class="step-body"><strong>Daftar Akun</strong><span>Isi nama, email, dan password</span></div>
          </div>
          <div class="step">
            <div class="step-ico" style="background:rgba(96,165,250,0.22);"><i class="fas fa-search" style="color:#93c5fd;"></i></div>
            <div class="step-body"><strong>Cari Buku</strong><span>Browse ratusan koleksi dari 10 genre</span></div>
          </div>
          <div class="step">
            <div class="step-ico" style="background:rgba(74,222,128,0.22);"><i class="fas fa-book-open" style="color:#86efac;"></i></div>
            <div class="step-body"><strong>Pinjam &amp; Baca</strong><span>1 klik langsung bisa baca online</span></div>
          </div>
          <div class="step">
            <div class="step-ico" style="background:rgba(244,114,182,0.22);"><i class="fas fa-trophy" style="color:var(--pk2);"></i></div>
            <div class="step-body"><strong>Kumpulkan Badge</strong><span>Raih badge dari aktivitas membaca</span></div>
          </div>
        </div>
      </div>
