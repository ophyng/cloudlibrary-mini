...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($aksi === 'pinjam' && !$pinjam_aktif && $buku['stok'] > 0) {
        ...
        if ($jml >= $max) { $pesan = "Kamu sudah meminjam $max buku sekaligus. Kembalikan dulu sebelum meminjam lagi."; $pesan_type = 'warning'; }
            ...
            $pdo->prepare("INSERT INTO peminjaman (id,user_id,buku_id,tanggal_pinjam,tanggal_expired,status,created_at) VALUES(?,?,?,?,?,'aktif',NOW())")->execute([$newPinjamId,$user_id,$buku_id,$tgl_pinjam,$tgl_expired]);
    ...
    elseif ($aksi === 'antre' && !$pinjam_aktif && !$antrian) {
        $newAntrianId = (int)$pdo->query("SELECT COALESCE(MAX(id),0)+1 FROM antrian")->fetchColumn();
        $pdo->prepare("INSERT INTO antrian (id,user_id,buku_id,tanggal_daftar) VALUES(?,?,?,NOW())")->execute([$newAntrianId,$user_id,$buku_id]);
    ...
    elseif ($aksi === 'wishlist') {
        if ($in_wishlist) { $pdo->prepare("DELETE FROM wishlist WHERE user_id=? AND buku_id=?")->execute([$user_id,$buku_id]); $pesan="Dihapus dari Wishlist."; $pesan_type='info'; }
            ...
            $newWishId = (int)$pdo->query("SELECT COALESCE(MAX(id),0)+1 FROM wishlist")->fetchColumn();
            $pdo->prepare("INSERT INTO wishlist (id,user_id,buku_id) VALUES(?,?,?)")->execute([$newWishId,$user_id,$buku_id]);
            $pesan="Ditambahkan ke Wishlist!"; $pesan_type='success';
        }
        header("Refresh:1;url=".BASE_URL."/mahasiswa/detail_buku.php?id=$buku_id");
    }
    elseif ($aksi === 'review') {
        ...
        if ($rating >= 1 && $rating <= 5 && $komentar) {
            if ($review_user) { $pdo->prepare("UPDATE review SET rating=?,komentar=? WHERE id=?")->execute([$rating,$komentar,$review_user['id']]); }
                ...
                $newReviewId = (int)$pdo->query("SELECT COALESCE(MAX(id),0)+1 FROM review")->fetchColumn();
                $pdo->prepare("INSERT INTO review (id,user_id,buku_id,rating,komentar) VALUES(?,?,?,?,?)")->execute([$newReviewId,$user_id,$buku_id,$rating,$komentar]);
                tambahPoin($pdo,$user_id,10);
            }
...
<div class="detail-outer">
  ...
  <div class="detail-wrap fu2">
    ...
    <div>
      ...
      <p class="info-penulis"><i class="fas fa-pen-nib" style="font-size:11px;color:var(--d3);"></i> <?= e($buku['penulis']) ?></p>
      ...
      <div class="meta-grid">
        <div class="meta-item"><div class="lbl"><i class="fas fa-tag"></i> Genre</div><div class="val"><?= e($buku['genre']) ?></div></div>
        <div class="meta-item"><div class="lbl"><i class="fas fa-calendar-alt"></i> Tahun</div><div class="val"><?= $buku['tahun'] ?: '-' ?></div></div>
        <div class="meta-item"><div class="lbl"><i class="fas fa-globe"></i> Bahasa</div><div class="val"><?= e($buku['bahasa'] ?: 'Indonesia') ?></div></div>
        <div class="meta-item"><div class="lbl"><i class="fas fa-hand-holding"></i> Dipinjam</div><div class="val"><?= $buku['total_dipinjam'] ?>x</div></div>
      </div>
      ...
      <div class="pinjam-bar" style="border-color:<?= $warn?'rgba(217,119,6,0.35)':'rgba(22,163,74,0.30)' ?>;">
        ...
        <div style="flex:1;">
          <div style="font-size:12px;color:var(--muted);font-weight:600;display:flex;align-items:center;gap:5px;margin-bottom:3px;"><i class="fas fa-calendar-plus" style="font-size:10px;"></i> Dipinjam <?= formatTanggal($pinjam_aktif['tanggal_pinjam']) ?></div>
          <div style="font-size:12px;color:var(--muted);font-weight:600;display:flex;align-items:center;gap:5px;"><i class="fas fa-calendar-times" style="font-size:10px;"></i> Jatuh tempo <?= formatTanggal($pinjam_aktif['tanggal_expired']) ?></div>
          <?php if($warn): ?>
          <div style="font-size:12px;color:#d97706;font-weight:800;margin-top:4px;display:flex;align-items:center;gap:5px;"><i class="fas fa-exclamation-triangle" style="font-size:10px;"></i> Hampir habis!</div>
          <?php endif; ?>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <a href="<?= BASE_URL ?>/mahasiswa/baca.php?id=<?= $buku_id ?>" class="btn-pinjam-main hijau"><i class="fas fa-book-open"></i> Baca Sekarang</a>
          <a href="<?= BASE_URL ?>/mahasiswa/perpanjang.php?id=<?= $pinjam_aktif['id'] ?>" class="btn-pinjam-main perpanjang"><i class="fas fa-redo"></i> Perpanjang</a>
          <a href="<?= BASE_URL ?>/mahasiswa/kembalikan.php?id=<?= $pinjam_aktif['id'] ?>" class="btn-pinjam-main" style="background:rgba(220,38,38,0.10);border:1.5px solid rgba(220,38,38,0.25)!important;color:#dc2626;" onclick="return confirm('Kembalikan buku ini sekarang? Akses akan langsung ditutup.')"><i class="fas fa-undo"></i> Kembalikan</a>
        </div>
      </div>
      <?php elseif($antrian): ?>
      <div class="pinjam-bar" style="border-color:rgba(217,119,6,0.28);">
        ...
        <div>
          ...
          <div style="font-size:12px;color:var(--muted);margin-top:3px;font-weight:600;"><i class="fas fa-calendar-alt" style="font-size:10px;"></i> Daftar sejak <?= formatTanggal($antrian['tanggal_daftar']) ?></div>
        </div>
      </div>
      <?php else: ?>
      <div style="display:flex;gap:10px;flex-wrap:wrap;">
        <?php if($tersedia): ?>
          <form method="POST" style="display:inline;">
            ...
            <button type="submit" class="btn-pinjam-main biru"><i class="fas fa-hand-holding"></i> Pinjam Sekarang</button>
          </form>
        <?php else: ?>
          <form method="POST" style="display:inline;">
            <input type="hidden" name="aksi" value="antre">
            <button type="submit" class="btn-pinjam-main oren"><i class="fas fa-list-ol"></i> Masuk Antrian</button>
          </form>
          <span style="display:inline-flex;align-items:center;gap:6px;padding:12px 20px;border-radius:100px;background:rgba(220,38,38,0.10);border:1.5px solid rgba(220,38,38,0.25);color:#dc2626;font-size:13px;font-weight:800;"><i class="fas fa-times-circle"></i> Stok Habis</span>
        <?php endif; ?>
      </div>
      <?php endif; ?>
    </div>
  ...
  <div class="section-block fu3">
    <div class="section-title"><i class="fas fa-star"></i> Ulasan Pembaca <span>(<?= $jumlah_review ?> ulasan)</span></div>
    <?php
    ...
    <div style="background:rgba(255,255,255,0.55);border:1px solid rgba(30,58,95,0.12);border-radius:14px;padding:18px;margin-bottom:20px;backdrop-filter:blur(10px);">
      ...
      <form method="POST">
        ...
        <button type="submit" class="btn-submit-sm"><i class="fas fa-paper-plane"></i> Kirim Ulasan</button>
      </form>
    </div>
    <?php else: ?>
    <div class="alert alert-info" style="margin-bottom:16px;"><i class="fas fa-info-circle"></i> Pinjam buku ini terlebih dahulu untuk memberikan ulasan.</div>
    <?php endif; ?>
    <?php if($reviews): ?>
  ...
  <div class="section-block fu4">
    <div class="section-title"><i class="fas fa-layer-group"></i> Buku Serupa <span>genre <?= e($buku['genre']) ?></span></div>
      ...
      <a href="<?= BASE_URL ?>/mahasiswa/detail_buku.php?id=<?= $s['id'] ?>" class="serupa-card">
        <div class="serupa-cover" style="background:linear-gradient(135deg,<?= $gws['bg'] ?>,<?= $gws['bg'] ?>bb);">
          <?php if(!empty($s['cover'])): ?>
            <img src="<?= BASE_URL ?>/uploads/covers/<?= e($s['cover']) ?>" style="position:absolute;inset:0;width:100%;height:100%;object-fit:contain;z-index:1;" alt="">
          <?php else: ?>
